...
per_h1n1_11r_e1 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/h1n1_persistence_11region/epoch1/out.stats") %>%
  mutate(type = "H1N1", epoch = "epoch1", geo_num = "11_region")
per_h1n1_11r_e2 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/h1n1_persistence_11region/epoch2/out.stats") %>%
  mutate(type = "H1N1", epoch = "epoch2", geo_num = "11_region")
per_h1n1_11r_e3 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/h1n1_persistence_11region/epoch3/out.stats") %>%
  mutate(type = "H1N1", epoch = "epoch3", geo_num = "11_region")

per_h3n2_11r_e1 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/h3n2_persistence_11region/epoch1/out.stats") %>%
  mutate(type = "H3N2", epoch = "epoch1", geo_num = "11_region")
per_h3n2_11r_e2 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/h3n2_persistence_11region/epoch2/out.stats") %>%
  mutate(type = "H3N2", epoch = "epoch2", geo_num = "11_region")
per_h3n2_11r_e3 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/h3n2_persistence_11region/epoch3/out.stats") %>%
  mutate(type = "H3N2", epoch = "epoch3", geo_num = "11_region")

per_bv_11r_e1 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/bv_persistence_11region/epoch1/out.stats") %>%
  mutate(type = "BV", epoch = "epoch1", geo_num = "11_region")
per_bv_11r_e2 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/bv_persistence_11region/epoch2/out.stats") %>%
  mutate(type = "BV", epoch = "epoch2", geo_num = "11_region")
per_bv_11r_e3 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/bv_persistence_11region/epoch3/out.stats") %>%
  mutate(type = "BV", epoch = "epoch3", geo_num = "11_region")

per_by_11r_e1 <- read.delim("//wsl.localhost/Ubuntu-22.04/home/zhiyuanchen/PACT-0.9.5/by_persistence_11region/epoch1/out.stats") %>%
...
#===lineage associated persistence==
#h3n2(10-22)
h3n2_file <- list.files(path = "../phylogeography/4.1post-analyses/persistence_h3n2_month_11region/", full.names = T)[str_detect(list.files(path = "../phylogeography/4.1post-analyses/persistence_h3n2_month_11region/", full.names = T),"h3n2")]
h3n2_p <- c()
for (i in 1:length(h3n2_file)) {
  # i = 1
  tmp <- read.delim(h3n2_file[i], sep = ",")
  tmp <- tmp %>%
    group_by(treeId, evaluationTime, stateAtEvaluationTime) %>%
    summarise(No_lineage = n(),
              persistenceTime = mean(persistenceTime)) %>%
    group_by(evaluationTime, stateAtEvaluationTime) %>%
    summarise(No_lineage = mean(No_lineage),
              # Persistent_lower = summarise_hpd_lower(persistenceTime),
              # Persistent_upper = summarise_hpd_upper(persistenceTime),
              Persistent_mean = mean(persistenceTime),
              Persistent_1q = quantile(persistenceTime,0.25),
              Persistent_3q = quantile(persistenceTime, 0.75),
              Persistent_median = median(persistenceTime))
  h3n2_p <- rbind(h3n2_p, tmp)
}
...
#==plot fot result of regression model== 
ag <- read.csv("../phylogeography/5.1regression-analyses/antigenic_effect_sumary.csv") %>% filter(year != 2017)
npi <- read.csv("../phylogeography/5.1regression-analyses/air-traffic_effect_sumary.csv") %>% filter(year != 2017)
