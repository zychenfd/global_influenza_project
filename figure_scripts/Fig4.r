...
value = c("9.Japan/Korea" = colors[1],"7.WesternAsia" = colors[3],"4.NorthernAmerica" = colors[6],
          ...
          "1.Africa"= colors[9], "Americas" = colors1[1],"Asia" = "#D8BFD8", "China" = colors1[4])
...
#==read data==
#under sub-sampling strategy 1
trunk_h1n1_even <- read.delim("../genomic_part/post-analyses/trunk/out_h1n1_even_1.skylines") %>% mutate(type = "H1N1")
trunk_h3n2_even <- read.delim("../genomic_part/post-analyses/trunk/out_h3n2_even_1.skylines") %>% mutate(type = "H3N2")
trunk_bv_even <- read.delim("../genomic_part/post-analyses/trunk/out_bv_even_1.skylines") %>% mutate(type = "BV")
trunk_by_even <- read.delim("../genomic_part/post-analyses/trunk/out_by_even_1.skylines") %>% mutate(type = "BY")
...
#under sub-sampling strategy 2
trunk_h1n1_tempo <- read.delim("../genomic_part/post-analyses/trunk/out_h1n1_tempo_1.skylines") %>% mutate(type = "H1N1")
trunk_h3n2_tempo <- read.delim("../genomic_part/post-analyses/trunk/out_h3n2_tempo_1.skylines") %>% mutate(type = "H3N2")
trunk_bv_tempo <- read.delim("../genomic_part/post-analyses/trunk/out_bv_tempo_1.skylines") %>% mutate(type = "BV")
trunk_by_tempo <- read.delim("../genomic_part/post-analyses/trunk/out_by_tempo_1.skylines") %>% mutate(type = "BY")
...
#average two sub-sampling strategies
...
#==output==
svg("output/Fig4.svg",width = 9, height = 8)
...
pdf("output/Fig4.pdf",width = 9, height = 8)
((p1 + p2 + p3 + p4) +plot_annotation(tag_level = "a") + plot_layout(nrow = 4,ncol = 1,guides = "collect",heights = c(1,1,1))&theme(legend.position = "right"))
dev.off()

tiff("output/Fig4.tif",width = 9, height = 8, units = "in", res = 400, compression = "lzw")
((p1 + p2 + p3 + p4) +plot_annotation(tag_level = "a") + plot_layout(nrow = 4,ncol = 1,guides = "collect",heights = c(1,1,1))&theme(legend.position = "right"))
dev.off()
