...
#==read map data==
#1.global map (we didn't provide the global map data as below due to the data sharing agreement)
world_region <- read.csv("../data_part/map_data/world_region.csv") # countries that we focused on in phylo-analyses
nine <- st_read("nine.shp")
nine <- st_transform(nine, CRS("+proj=longlat +ellps=WGS84 +towgs84=0,0,0,0,0,0,0 +no_defs"))
worldmap0 <- st_read("World_map.shp")
...
#2.china map
china_province <- read_xlsx("../data_part/map_data/province.xlsx")
china <- st_read("../data_part/map_data/china_map/gadm41_CHN_1.shp")
china_taiwan <- st_read("../data_part/map_data/china_map/gadm41_TWN_0.shp")
...
list.files("OAG_DATA/AIRPOT2AIRPOT", full.names = T) -> tmp_files
tmp_files <- tmp_files[c(1:(length(tmp_files)-1))]

#CHINA airport (help divide China into north China and South China)
china_province <- read_xlsx("../data_part/map_data/province.xlsx")
china_air <- read_xlsx("China_airport.xlsx") %>% 
...
#==Air traffic comparison between each pair of regions across epochs (panel e-f)==
air_data1 <- air_data 
